...
__global__ void count_kernel(const int* input, int* output, long long total, int P) {
...
extern "C" void solve(const int* input, int* output, int N, int M, int K, int P) {
    long long total = (long long)N * M * K;
    int threadsPerBlock = 256;
    long long blocksPerGrid = (total + threadsPerBlock - 1) / threadsPerBlock;
    if (blocksPerGrid > 4096) blocksPerGrid = 4096;

    cudaMemset(output, 0, sizeof(int));
    count_kernel<<<(int)blocksPerGrid, threadsPerBlock>>>(input, output, total, P);
